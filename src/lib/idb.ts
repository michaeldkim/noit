  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(META_STORE)) {
        meta = db.createObjectStore(META_STORE, {
          keyPath: "id",
          autoIncrement: true,
        });
      } else {
      ...
      if (!db.objectStoreNames.contains(NOTES_STORE)) {
        notes = db.createObjectStore(NOTES_STORE, {
          keyPath: "id",
          autoIncrement: true,
        });
      } else {
...
export async function addFile(file: File): Promise<AddResult> {
  ...
  const idKey = await withReq<IDBValidKey>(
    db1
    ...
    try {
      ...
      await withReq(
        db2
          .transaction(META_STORE, "readwrite")
          .objectStore(META_STORE)
          .put({ ...base, id, location: loc } as any)
      );
      db2.close();
      return { id };
    } catch {
  ...
  await withReq(
    tx3
      .objectStore(META_STORE)
      .put({ ...base, id, storage: "idb", location: `idb:${id}` } as any)
  );
...
export async function getFileBlob(id: number): Promise<Blob | null> {
  ...
  if (meta.storage === "opfs") {
    try {
      return await readOPFSFile(id);
    } catch {}
  }
  const db = await openDB();
  const rec = await withReq<any>(
    db.transaction(BLOB_STORE, "readonly").objectStore(BLOB_STORE).get(id)
  );
  db.close();
...
export async function deleteFile(id: number): Promise<void> {
  ...
  if (meta?.storage === "opfs") {
    ...
    await withReq(
      dbb
        .transaction(BLOB_STORE, "readwrite")
  ...
  const db = await openDB();
  await withReq(
    db.transaction(META_STORE, "readwrite").objectStore(META_STORE).delete(id)
  );
  db.close();
}
...
export async function deleteAllInEnv(env: string): Promise<void> {
  ...
  const db3 = await openDB();
  const notes = await withReq<any[]>(
    db3.transaction(NOTES_STORE, "readonly").objectStore(NOTES_STORE).getAll()
  );
  const noteIds = notes
    .filter((n) => (n.env ?? "main") === env)
    .map((n) => n.id);
  db3.close();
  if (noteIds.length) {
    ...
    await new Promise<void>((res, rej) => {
      tx4.oncomplete = () => res();
      tx4.onerror = () => rej(tx4.error);
      tx4.onabort = () => rej(tx4.error);
    });
    db4.close();
  }
}
// counts for confirm modal (no env index yet, filter in-memory)
export async function countTotalsInEnv(
  env: string
): Promise<{ files: number; notes: number }> {
  const db = await openDB();
  const tx1 = db.transaction(META_STORE, "readonly");
  const s1 = tx1.objectStore(META_STORE);
  let files = 0;
  try {
    files = await withReq<number>(s1.index("env").count(env));
  } catch {
  ...
  let notes = 0;
  try {
    notes = await withReq<number>(s2.index("env").count(env));
  } catch {
...
export async function listNotesByKind(kind: string, env?: string) {
  ...
  try {
    ...
    db.close();
    return (rows as any).filter((n: any) =>
      env ? (n.env ?? "main") === env : true
    );
  } catch {
    const rows = await withReq<any[]>(store.getAll());
    db.close();
...
export async function deleteNote(id: number): Promise<void> {
  ...
  try {
    const tx = db.transaction(NOTES_STORE, "readwrite");
    const store = tx.objectStore(NOTES_STORE);
    await withReq(store.delete(id));
  } finally {
    db.close();
  }
}

// toggle a to-do note's done flag and update updatedAt
export async function setTodoDone(id: number, done: boolean): Promise<void> {
  const db = await openDB();
  try {
    const tx = db.transaction(NOTES_STORE, "readwrite");
    const store = tx.objectStore(NOTES_STORE);
    const note: any = await withReq<any>(store.get(id));
    if (!note) return;
    let body: any = {};
    try {
      body = note.body ? JSON.parse(note.body) : {};
    } catch {
      body = {};
    }
    body.done = done;
